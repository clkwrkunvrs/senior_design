...
class vnaStuff():
    ...
    def getPosition(self, wave1, wave2, wave3, range1, range2, range3):
        ...
        if self.meetsThreshold(wave1) and self.meetsThreshold(wave2) and self.meetsThreshold(wave3):
            ...
            range1_calc = light * range1 - self.range_calc - 16.2
            ...
            xRangeLeft = numpy.linspace(minLeft, maxLeft,
                                        200)  # always do 100 points. Can adjust to make the plot look nicer.
            ...
            if (maxLeft < minRight):  # non intersecting
                ...
                self.az_calc = numpy.arctan2(y_tar, x_tar) * 180 / numpy.pi
        ...
        # If true, means out of cone for range, uses bistatic method to calc range + angle
            ...
            range_adj = 0
            range1_calc = (light * range1) / 4 - range_adj
            range2_calc = (light * range2) / 4 - range_adj
    ...
    def plotPosition(self, currentTheta=0, currentR=0):
        ...
        offset = 9
        half_beam = 5
        theta = [0, math.radians(currentTheta + offset)]
        thetaLeft = [0, math.radians(currentTheta - half_beam + offset)]
        thetaRight = [0, math.radians(currentTheta + half_beam + offset)]
        r = [0, currentR]
        # clear plot and prep for redraw
        self.axPolar.cla()
        self.axPolar.set_title('Object Position')
        # set limit on polar plot range
        self.axPolar.set_ylim([0, 10])
        # tell the user what the range is at the moment in the bottom left corner
        self.axPolar.annotate(
            'Range: {:0.2f}'.format(currentR) + " Feet\nAngle: {:0.2f}".format(currentTheta) + ' degrees',
        ...
        self.axPolar.plot(thetaLeft, r, '-b')
        self.axPolar.plot(thetaRight, r, '^b')
        self.axPolar.plot(thetaRight, r, '-b')
    ...
    def pullTrigger(self):
        global ser
        triggerPulled = False
        if 110 > self.az_calc > 80:
            ser.write(b'1')
            ser.readline()
            #while (message != b'done\r\n'):
